...
const AllDevices = () => {
	...
	useEffect(() => {
		...
		fetchDeviceList(window.sessionStorage.getItem("refreshToken")).then(
			(response) => {
			...
			<>
				<div className="container w-auto px-5 py-2 bg-slate-600">
					<div className="bg-white mt-4 p-6">
						...
						<label
							className="block px-2 font-bold text-gray-500"
							style={{ fontFamily: "Poppins, sans-serif" }}
						>
							{t("logotofeachdevice")}
						</label>
						...
						{devices && devices.length > 0 ? (
							<>
								{devices.map((device) => (
									...
									>
										<div className="border bg-gray-200 items-center sm:items-start space-y-2 sm:space-y-0 border-gray-300 transition-all duration-300 hover:bg-red-200 hover:shadow-lg transform hover:-translate-y-1 rounded-md p-2.5 flex flex-col sm:flex-row w-full sm:space-x-4">
											<svg
