...
import { productRequestStatus,productRequestStatusByName } from "../../Constants/containerStatus";
...
function AllProductRequests() {
    ...
    const [status, setStatus] = useState(productRequestStatusByName.pending);
    const [stores, setStores] = useState([]);
    const [cityId, setCityId] = useState();


    const fetchProductRequests = useCallback((storeId, pageNumber, status) => {
        if (storeId) {
            getProductRequests(storeId, pageNumber, status).then((response) => {
    ...
    useEffect(() => {
        if (storeId) {
            ...
            if (selectedStore) {
                fetchProductRequests(storeId, pageNumber, status);
            }
        }
    }, [fetchProductRequests,status,storeId,pageNumber,stores]);
    ...
    const handleStoreChange = async (event) => {
        ...
        if (selectedStore) {
            ...
            fetchProductRequests(storeId, 1); // Reset to first page
        }
    };
    ...
    function getStatusClass(statusId) {
        if (productRequestStatus[statusId] === "Accepted") {
            return "bg-green-400";
        }
        if (productRequestStatus[statusId] === "Rejected") {
            return "bg-red-400";
        }
        if (productRequestStatus[statusId] === "Pending") {
            return "bg-yellow-400";
        }
        if (productRequestStatus[statusId] === "Changerequested") {
            return "bg-green-400";
        }
    }
                        ...
                        <div className="w-full h-full flex items-center lg:py-2 py-5 justify-end xl:justify-center lg:justify-center border-gray-100 md:space-x-10">
                            <div className="hidden lg:block">
                                <div className="w-full h-full flex items-center justify-end xl:justify-center lg:justify-center md:justify-end sm:justify-end border-gray-100 md:space-x-10">
                                    <div
                                        ...
                                        onClick={() => setStatus(productRequestStatusByName.pending)}
                                    ...
                                    <div
                                        className="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md p-4 text-sm font-bold cursor-pointer"
                                        onClick={() => setStatus(productRequestStatusByName.accepted)}
                                        style={{ fontFamily: "Poppins, sans-serif" }}
                                    >
                                        {t("pending")}
                                    </div>
                                    <div
                                        className="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md p-4 text-sm font-bold cursor-pointer"
                                        onClick={() => setStatus(productRequestStatusByName.rejected)}
                                        style={{ fontFamily: "Poppins, sans-serif" }}
                                    >
                                        {t("inactive")}
                                    </div>
                                    <div
                                        className="text-gray-300 hover:bg-gray-700 hover:text-white rounded-md p-4 text-sm font-bold cursor-pointer"
                                        onClick={() => setStatus(productRequestStatusByName.changeRequested)}
                                        style={{ fontFamily: "Poppins, sans-serif" }}
                                    >
                                        {t("inactive")}
                                    </div>
                            ...
                            <div className="-my-2 -mr-2 lg:hidden">
                                <select
                                    ...
                                    style={{ fontFamily: "Poppins, sans-serif" }}
                                >
                                    <option value={productRequestStatusByName.Active}>{t("active")}</option>
                                    <option value={productRequestStatusByName.Pending}>{t("pending")}</option>
                                    <option value={productRequestStatusByName.Inactive}>{t("inactive")}</option>
                                    <option value={productRequestStatusByName.Inactive}>{t("inactive")}</option>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center justify-center">
                                                        ...
                                                        <h1 style={{ fontFamily: "Poppins, sans-serif" }}>
                                                            {t(productRequestStatus[product.status].toLowerCase())}
                                                        </h1>
                                                    </div>
                                                </td>
