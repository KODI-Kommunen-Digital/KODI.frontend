...
import forumStatus from "../../Constants/forumStatus";
...
const Forum = () => {
	...
	async function checkUserMembership() {
		...
		document.title = `${process.env.REACT_APP_NAME} - Forums`;
		if (cityId && forumId) {
			getForum(cityId, forumId).then((response) => {
			...
			try {
				...
				if (forum) {
					...
					setIsOnlyMember(membersResponse.data.data.length === 1);
					setPageNo(pageNoParam);
				} else {
					const membersResquests = await getUserForumRequests({ statusId: forumStatus.Pending, cityId });
					const membersResquest = membersResquests.data.data.find(r => r.forumId === forumId)
					if (membersResquest) {
						setRequestId(membersResquest.id)
					}
				}
			} catch (error) {
	...
	const handleFollow = async () => {
		try {
			await getForumMemberRequests(cityId, forumId);
			setMemberStatus(true);
	...
	const handleFollowRequest = async () => {
		try {
			const memberRequest = await getForumMemberRequests(cityId, forumId);
			setRequestId(memberRequest.data.data.id);
	...
	const handleCancelRequest = async () => {
		try {
			await cancelForumMemberRequest(cityId, forumId, requestId);
			setRequestId(0);
			localStorage.setItem("followRequested", "true");
		} catch (error) {
			console.error("Error sending follow request:", error);
					...
					<div>
						...
						<div className="text-center justify-between lg:px-10 md:px-5 sm:px-0 px-4 md:py-6 py-4 bg-gray-50">
							{forums.isPrivate && !memberStatus ? (
								<a
									onClick={requestId ? handleCancelRequest : handleFollowRequest}
